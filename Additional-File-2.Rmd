...
The figure provides a descriptive overview of sulfide concentrations (mean ± SE) at baseline (T0) and after exposure (T2) for sediments colonized by Caulerpa (blue) and Cymodocea (orange) under control (solid lines) and heatwave (dashed lines) conditions. Visually, sulfide levels increased from T0 to T2 in both species. The increase was apparent under both control and heatwave treatments in Caulerpa, whereas in Cymodocea the rise appeared more pronounced under heatwave conditions. Individual data points (faded symbols) illustrate within-group variability.

To formally assess these visual patterns, we applied a nonparametric framework. We first report group medians at T0 and T2 and their difference ($\Delta = T2 - T0$) for each Species $\times$ Treatment as descriptive summaries. We then tested within-group temporal changes (T0 vs T2) separately for each Species $\times$ Treatment using Wilcoxon tests, with p-values adjusted using the Benjamini–Hochberg procedure. Finally, we compared treatments at T2 within each species to assess whether heatwave exposure altered sulfide concentrations at the final time point.

```{r}
medians <- datos %>%
  group_by(Species, Treatment, Time) %>%
  summarise(median_sulfide = median(Sulfur, na.rm = TRUE), n = dplyr::n(), .groups = "drop") %>%
...
We then formally tested T0 vs T2 within each Species × Treatment using Wilcoxon rank–sum tests, controlling the false discovery rate with Benjamini–Hochberg adjustment. This step separates the inferential result from the descriptive trends shown in the figure.
...
Together, the descriptive summaries and Wilcoxon tests support the interpretation that sulfide increased over time in both species, while the heatwave-related increase appeared stronger in Cymodocea at T2. The DiD models below then evaluate whether that apparent treatment-related amplification is supported when time and treatment are modelled jointly.


#### Difference-in-Differences (DiD) models

To complement the nonparametric analyses and jointly assess the effects of time and treatment, a difference-in-differences (DiD) model was applied separately for each species. This approach makes it possible to test whether the temporal change in sulfide differs significantly between control and heatwave conditions, while accounting for the main effects of treatment and time. In particular, the interaction term (Treatment × Time) captures the differential effect attributable to the heatwave at the final time point (T2), thus providing a direct measure of the magnitude of the treatment impact in each species.
...
In Caulerpa, sulfide concentrations increased significantly from T0 to T2 under control conditions (Time effect: p < 0.001). The DiD model further estimated an additional increase under heatwave exposure at T2 (interaction coefficient = 114.9), consistent with the descriptive pattern of greater accumulation under heatwave conditions. However, this differential effect was not statistically significant (p = 0.17), indicating that while sulfide accumulation clearly occurred over time, the evidence for an extra increase driven by heatwave exposure remains moderate.
...
The figure provides a descriptive overview of sediment redox potential (mean ± SE) at baseline (T0) and after exposure (T2) for sediments colonized by Caulerpa (blue) and Cymodocea (orange) under control (solid lines) and heatwave (dashed lines) conditions. Visually, redox potential tended to decrease from T0 to T2 in both species. In Caulerpa, the decline appeared more evident under control conditions than under heatwave conditions. In Cymodocea, redox potential also appeared to decrease over time, with a stronger visual decline under heatwave treatment.

To formally assess these visual patterns, we applied a nonparametric framework. We evaluated within-group changes from T0 to T2 for each Species $\times$ Treatment using Wilcoxon tests and then compared treatments at T2 within each species. P-values were adjusted for multiple testing using the Benjamini–Hochberg procedure.
...
These summaries indicate that redox potential generally decreased over time, with the largest apparent declines occurring in Caulerpa under control conditions and in Cymodocea under heatwave conditions. The formal Wilcoxon results below determine which of these visual shifts are statistically supported.
...
We analyzed cell counts in sediments colonized by either Caulerpa or Cymodocea under control and heatwave conditions. In this section, we first describe the temporal patterns (T0–T1–T2) within each Species × Treatment group and then report the corresponding inferential comparisons, followed by between-species contrasts.
...
This overall comparison indicated that cell counts differed between species, with Caulerpa generally showing higher values than Cymodocea across the experiment.
